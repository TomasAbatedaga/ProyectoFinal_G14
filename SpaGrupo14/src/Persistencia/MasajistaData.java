...
public class MasajistaData {
    ...
    public List<Masajista> listarMasajistasDisponibles(LocalDate fecha, Time horaInicio, Time horaFin) {
        List<Masajista> disponibles = new ArrayList<>();

        String sql = "SELECT * FROM masajista " +
             "WHERE estado = 1 " +
             "AND cod_masajista NOT IN (" +
             "    SELECT s.cod_masajista " +
             "    FROM sesion s " +
             "    JOIN dia_de_spa d ON s.cod_pack = d.cod_pack " +
             "    WHERE d.fecha_hora = ? " +
        ...
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            System.out.println("Error al listar masajistas disponibles: " + ex.getMessage());
        }
        return disponibles;
    }
